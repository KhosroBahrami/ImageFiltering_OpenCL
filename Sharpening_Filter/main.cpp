...
int main(int argc, char * argv[])
{
    ...
    // Load kernel
    const char * kernelFile = "sharpeningFilter.cl";
    ...
    int mask_width = 3;
    int mask_height = 3;
    const float average_mask[9] = {-1, -1, -1, -1, 9, -1, -1, -1, -1};
    ...
    cl_kernel kernel = clCreateKernel(program,"sharpeningFilter",&status);
    if(status != CL_SUCCESS){
    ...
    sharpeningFilter_CPU(src, dstImage);
    // Output image
