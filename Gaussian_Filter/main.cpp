...
int main(int argc, char * argv[])
{
    ...
    // Load kernel
    const char * kernelFile = "gaussianFilter.cl";
    ...
    int mask_width = 3;
    int mask_height = 3;
    const float average_mask[9] = {1 / 16.0f,2 / 16.0f,1 / 16.0f,
    ...
    cl_kernel kernel = clCreateKernel(program,"gaussianFilter",&status);
    if(status != CL_SUCCESS){
    ...
    // run gaussian filter on CPU  
    gaussianFilter_CPU(src, dstImage);
    // Output image
